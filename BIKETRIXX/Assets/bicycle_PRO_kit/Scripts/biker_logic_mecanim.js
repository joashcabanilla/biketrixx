...
//for legs
// variables for turn IK link off for a time
private var IK_rightLegWeight : float = 1;
private var IK_leftLegWeight : int = 1;
...
//for legs
// variables for hand IK joint points
var IK_rightLegTarget :  Transform;
var IK_leftLegTarget :  Transform;
...
private var bikeStatusCrashed : bicycle_code;// making a link to corresponding bike's script
//why it's here ? because you might use this script with many bikes in one scene
...
function Update () {
	...
	if (Mathf.Round((bikeRideOn.GetComponent.<Rigidbody>().velocity.magnitude * 3.6)*10) * 0.1 <= 5 && !bikeStatusCrashed.isReverseOn){//no reverse speed
	reverseSpeed = 0.0;
	myAnimator.SetFloat("reverseSpeed", reverseSpeed);
		...
		if (bikeRideOn.transform.localEulerAngles.z <=15 || bikeRideOn.transform.localEulerAngles.z >=345){
			if (bikeRideOn.transform.localEulerAngles.x <=10 || bikeRideOn.transform.localEulerAngles.x >=350){
				var legOffValue = (5-(Mathf.Round((bikeRideOn.GetComponent.<Rigidbody>().velocity.magnitude * 3.6)*10) * 0.1))/5;//need to define right speed to begin put down leg(s)
		...
		if (IK_rightLegWeight < 1){
			IK_rightLegWeight = IK_rightLegWeight + 0.01;
		}
	}
	//when using reverse speed
	if (Mathf.Round((bikeRideOn.GetComponent.<Rigidbody>().velocity.magnitude * 3.6)*10) * 0.1 <= 5 && bikeStatusCrashed.isReverseOn){//reverse speed
	...
	if (Mathf.Round((bikeRideOn.GetComponent.<Rigidbody>().velocity.magnitude * 3.6)*10) * 0.1 > 5){
		reverseSpeed = 0.0;
		myAnimator.SetFloat("reverseSpeed", reverseSpeed);
		myAnimator.SetLayerWeight(3, legOffValue);
		myAnimator.SetLayerWeight(2, 0); //to turn off layer with reverse animation which override all other
